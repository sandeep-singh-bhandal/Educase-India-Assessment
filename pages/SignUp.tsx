...
const SignUp = () => {
  ...
  return (
    <section className="flex justify-center items-center w-screen h-screen">
      <form className="flex flex-col justify-between border-2 border-[#EBEBEB] hover:border-[#6C25FF] transition-all duration-300 w-1/4 h-full bg-[#F7F8F9] p-5 ">
        <div>
          <h1 className="text-[28px] w-2/3 text-[#1d2226] mb-8 font-[600]">
            {SignUpPageData.title}
          </h1>
          {SignUpPageData.fields.map((label, index) => (
            <div className="relative w-80 mt-4" key={index}>
